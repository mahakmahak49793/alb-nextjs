...
function AddTopicContent(){
  ...
  // FIXED: Improved getTopicData function
  const getTopicData = async (id: string) => {
    try {
      setFetching(true);
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/get_live_session_topic/${id}`);
      ...
      console.log('Topic API Response:', data); // Debug log
      ...
      if (data.success) {
        ...
        if (topicData) {
          setInputFieldDetail({
            categoryId: topicData.categoryId?._id || topicData.categoryId || '',
            title: topicData.topicName || topicData.title || ''
          });
        } else {
          throw new Error('No topic data found');
        }
      } else {
        throw new Error(data.message || 'Failed to fetch topic data');
      }
    } catch (error) {
      console.error('Error fetching topic:', error);
      Swal.fire({
  ...
  // Alternative API call if the above doesn't work
  const getTopicDataAlternative = async (id: string) => {
    try {
      setFetching(true);
      // Try to get all topics and filter by ID
      ...
      if (data.success) {
        ...
        const specificTopic = topics.find((topic: any) => topic._id === id);
        
        if (specificTopic) {
          setInputFieldDetail({
            categoryId: specificTopic.categoryId?._id || specificTopic.categoryId || '',
            title: specificTopic.topicName || specificTopic.title || ''
          });
        } else {
      ...
      console.error('Error fetching topic (alternative):', error);
      Swal.fire({
        icon: 'error',
        title: 'Error!',
        text: 'Failed to load topic data. Please try again.',
  ...
  // Fetch data on component mount - IMPROVED
  useEffect(() => {
    const fetchData = async () => {
      await getLiveSessionCategory();
      ...
      if (editMode && topicId) {
        ...
        // If that fails, the alternative method will be called automatically due to the error handling
      }
    };
